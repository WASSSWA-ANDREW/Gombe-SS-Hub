...
@section('title', 'Staff on Government List')
@section('header', 'Staff Members (Government)')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-white">Staff Directory (Government)</h2>
        <div>
            <a href="{{ route('admin.staff.export.excel_govt') }}" class="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-md shadow-sm mr-2">
                Export to Excel
            </a>
            <a href="{{ route('admin.staff.export.pdf_govt') }}" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-md shadow-sm mr-2">
                Export to PDF
            </a>
            <a href="{{ route('admin.staff.create_govt') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-md shadow-sm">
                Add New Staff (Govt)
            </a>
        </div>
    </div>
    ...
    <div class="mb-6">
        <div class="relative group">
            ...
            <form action="{{ route('admin.staff.govt.search') }}" method="GET" class="flex">
                <input type="text" name="query" id="govtStaffSearchInput"
                    ...
                    placeholder="Search staff by name, UTS/File No., IPPS No..."
                    value="{{ $query ?? '' }}"
                    autocomplete="off">
                ...
                @forelse ($staffMembers as $staff)
                    <tr class="staff-row">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                            <a href="{{ route('admin.staff.view.pdf_govt', $staff->id) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200 hover:underline" target="_blank">
                                {{ $staff->surname }}, {{ $staff->first_name }}
                            </a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300">
                            {{ $staff->uts_file_no ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300">
                            {{ $staff->ipps_no ?? 'N/A' }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('admin.staff.show_govt', $staff->id) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-200 mr-3">View</a>
                            <a href="{{ route('admin.staff.edit_govt', $staff->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-200 mr-3">Edit</a>
                            {{-- Add Export Form PDF for Govt staff if a specific form PDF view is created --}}
                            {{-- <a href="{{ route('admin.staff.export.form_pdf_govt', $staff->id) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-200 mr-3">Export Form PDF</a> --}}
                            <form action="{{ route('admin.staff.destroy_govt', $staff->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this staff member?');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300 text-center">
                            No government staff members found.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const searchInput = document.getElementById('govtStaffSearchInput');
        ...
        searchInput.addEventListener('keyup', function() {
            ...
            tableRows.forEach(row => {
                ...
                const utsFileNo = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
                const ippsNo = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
                const dateJoined = row.querySelector('td:nth-child(5)').textContent.toLowerCase();
                
                if (fullName.includes(searchTerm) || 
                    utsFileNo.includes(searchTerm) || 
                    ippsNo.includes(searchTerm) || 
                    dateJoined.includes(searchTerm)) {
                    row.style.display = '';
                } else {
            ...
            // Update selection status after search
        ...
        exportSelectedBtn.addEventListener('click', function() {
            ...
            form.method = 'POST';
            form.action = '{{ route("admin.staff.export.selected_govt") }}';
        ...
        deleteSelectedBtn.addEventListener('click', function() {
            ...
            if (!confirm(`Are you sure you want to delete ${selectedIds.length} government staff member(s)? This action cannot be undone.`)) {
                return;
            }
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("admin.staff.delete.selected_govt") }}';
            form.style.display = 'none';
