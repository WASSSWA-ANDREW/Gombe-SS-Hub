...
@section('title', 'O\'Level Student Details')
@section('header', 'O\'Level Student Details')
...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-700 dark:text-white">O'LEVEL STUDENT DETAILS</h2>
        ...
        <fieldset class="mb-6 border border-gray-300 dark:border-gray-600 p-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 mt-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-base text-gray-900 dark:text-white">{{ $student->class ?? 'N/A' }}</p>
                </div>
            </div>
        </fieldset>
        ...
        <fieldset class="mb-6 border border-gray-300 dark:border-gray-600 p-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 mt-4">
                @php
                    $pleResults = is_string($student->ple_results) ? json_decode($student->ple_results, true) : $student->ple_results;
                @endphp
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">English:</label>
                    <p class="mt-1 text-base text-gray-900 dark:text-white">{{ $pleResults['english'] ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Math:</label>
                    <p class="mt-1 text-base text-gray-900 dark:text-white">{{ $pleResults['math'] ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Science:</label>
                    <p class="mt-1 text-base text-gray-900 dark:text-white">{{ $pleResults['science'] ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">SST:</label>
                    <p class="mt-1 text-base text-gray-900 dark:text-white">{{ $pleResults['sst'] ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Aggregate:</label>
                    <p class="mt-1 text-base text-gray-900 dark:text-white font-semibold">{{ $student->ple_aggregate ?? 'N/A' }}</p>
                </div>
            </div>
        </fieldset>
