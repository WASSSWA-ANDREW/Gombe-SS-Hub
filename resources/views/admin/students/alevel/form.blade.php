...
@section('title', isset($student) ? "Edit A'Level Student" : "Add A'Level Student")
@section('header', isset($student) ? "Edit A'Level Student" : "Add A'Level Student")

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-white mb-6 text-center">A'LEVEL FORM</h2>
        ...
        <form action="{{ isset($student) ? route('admin.students.alevel.update', $student->id) : route('admin.students.alevel.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($student))
                @method('PUT')
            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 mt-4">
                    ...
                    <div>
                        ...
                        <select name="class" id="class" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900">
                            ...
                            <option value="S.5" {{ old('class', $student->class ?? '') == 'S.5' ? 'selected' : '' }}>S.5</option>
                            <option value="S.6" {{ old('class', $student->class ?? '') == 'S.6' ? 'selected' : '' }}>S.6</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="uce_index_number" id="uce_index_number" value="{{ old('uce_index_number', $student->uce_index_number ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900">
                    </div>
                    <div>
                    ...
                    <div>
                        @php
                        $combination = old('combination', $student->combination ?? '');
                        $predefined = ['PCM', 'PCB', 'BCM', 'HEG', 'HEL', 'MEG'];
                        $otherValue = old('other_combination', (!in_array($combination, $predefined) && $combination !== '') ? $combination : '');
                        $selected = in_array($combination, $predefined) ? $combination : 'Other';
                        @endphp
                        <label for="combination" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Subject Combination:</label>
                        <select name="combination" id="combination" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900">
                            <option value="">Select Combination</option>
                            <option value="PCM" {{ $selected == 'PCM' ? 'selected' : '' }}>PCM (Physics, Chemistry, Mathematics)</option>
                            <option value="PCB" {{ $selected == 'PCB' ? 'selected' : '' }}>PCB (Physics, Chemistry, Biology)</option>
                            <option value="BCM" {{ $selected == 'BCM' ? 'selected' : '' }}>BCM (Biology, Chemistry, Mathematics)</option>
                            <option value="HEG" {{ $selected == 'HEG' ? 'selected' : '' }}>HEG (History, Economics, Geography)</option>
                            <option value="HEL" {{ $selected == 'HEL' ? 'selected' : '' }}>HEL (History, Economics, Literature)</option>
                            <option value="MEG" {{ $selected == 'MEG' ? 'selected' : '' }}>MEG (Mathematics, Economics, Geography)</option>
                            <option value="Other" {{ $selected == 'Other' ? 'selected' : '' }}>Other (specify below)</option>
                        </select>
                        <div id="other-combination-container" style="display: {{ $selected === 'Other' ? 'block' : 'none' }};" class="mt-2">
                            <label for="other_combination" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Specify Other Combination:</label>
                            <input type="text" name="other_combination" id="other_combination" value="{{ $otherValue }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900" placeholder="e.g., PEM (Physics, Economics, Mathematics)">
                        </div>
                    </div>
            ...
            <fieldset class="mb-6 border border-gray-300 dark:border-gray-600 p-4 rounded-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 mt-4">
                    ...
                    <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Grade</label></div>

                    <div class="md:col-span-2"><input type="text" value="ENGLISH LANGUAGE" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_english" value="{{ old('uce_english', $student->uce_results['english'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>
                    ...
                    <div class="md:col-span-2"><input type="text" name="uce_mathematics" value="{{ old('uce_mathematics', $student->uce_results['mathematics'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="PHYSICS" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_physics" value="{{ old('uce_physics', $student->uce_results['physics'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="CHEMISTRY" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_chemistry" value="{{ old('uce_chemistry', $student->uce_results['chemistry'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="BIOLOGY" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_biology" value="{{ old('uce_biology', $student->uce_results['biology'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="HISTORY" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_history" value="{{ old('uce_history', $student->uce_results['history'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="GEOGRAPHY" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_geography" value="{{ old('uce_geography', $student->uce_results['geography'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="LITERATURE" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_literature" value="{{ old('uce_literature', $student->uce_results['literature'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><input type="text" value="ECONOMICS" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-base py-3 px-4 bg-gray-100 text-gray-700" readonly></div>
                    <div class="md:col-span-2"><input type="text" name="uce_economics" value="{{ old('uce_economics', $student->uce_results['economics'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-2"><label for="uce_aggregates" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mt-2">AGGREGATES:</label></div>
                    <div class="md:col-span-2"><input type="number" name="uce_aggregates" id="uce_aggregates" value="{{ old('uce_aggregates', $student->uce_results['aggregates'] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-base py-3 px-4 bg-white text-gray-900"></div>

                    <div class="md:col-span-4 mt-2">
                        <label for="result_slip" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Attach Photocopy of UCE Result Slip/Testimonial:</label>
                        <input type="file" name="result_slip" id="result_slip" class="mt-1 block w-full text-sm text-gray-800 dark:text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 dark:file:bg-gray-700 file:text-indigo-700 dark:file:text-indigo-300 hover:file:bg-indigo-100 dark:hover:file:bg-gray-600">
                        @if(isset($student) && $student->result_slip_path)
                        <p class="text-xs text-gray-800 mt-1">Current file: <a href="{{ asset('storage/' . $student->result_slip_path) }}" target="_blank" class="text-indigo-600 hover:underline">View Result Slip</a></p>
                        @endif
                    </div>
                </div>
            </fieldset>
...
document.getElementById('combination').addEventListener('change', function() {
    var container = document.getElementById('other-combination-container');
    if (this.value === 'Other') {
