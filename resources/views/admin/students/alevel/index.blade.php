...
@section('title', 'A\'Level Students List')
@section('header', 'A\'Level Students')
        ...
        <div class="px-6 py-4">
            ...
            <div class="mb-4">
                <div class="relative group">
                    ...
                    <form action="{{ route('admin.students.alevel.search') }}" method="GET" class="flex">
                        <input type="text" name="query" id="studentSearchInput"
                            ...
                            placeholder="Search students by name, LIN, subject combination, previous school..."
                            value="{{ $query ?? '' }}"
                            autocomplete="off">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($students as $student)
                                <tr class="student-row">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300">
                                        {{ $student->subject_combination ?? 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300">
            ...
            @else
                <p class="text-gray-700 dark:text-gray-300 mt-4">No A'Level students found.</p>
                <p class="text-gray-800 dark:text-gray-400 text-sm">
                    You can add a new A'Level student by clicking the "Add New A'Level Student" button above.
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('keyup', function() {
            ...
            tableRows.forEach(row => {
                ...
                const subjectCombo = row.querySelector('td:nth-child(5)').textContent.toLowerCase();
                const prevSchool = row.querySelector('td:nth-child(6)').textContent.toLowerCase();
                
                if (name.includes(searchTerm) || 
                    gender.includes(searchTerm) || 
                    lin.includes(searchTerm) || 
                    subjectCombo.includes(searchTerm) || 
            ...
            if (visibleRows.length === 0 && !noResultsRow) {
                ...
                newRow.innerHTML = `
                    <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-300 text-center">
        ...
        exportSelectedBtn.addEventListener('click', function() {
            ...
            const format = confirm('Click OK to export as Excel, or Cancel to export as PDF.') ? 'excel' : 'pdf';
            
            // Create a form to submit the selected IDs
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("admin.students.alevel.export.selected") }}';
